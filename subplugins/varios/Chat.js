...
async function isAdminByNumber(conn, chatId, number) {
  try {
    ...
    console.error("[chat] Error reading admins:", e);
    return false;
  }
}
...
const handler = async (msg, { conn }) => {
  try {
    ...
    if (!isAdmin && !isOwner && !isBotMsg) {
      await conn.sendMessage(chatId, { text: "🚫 Solo los administradores u owners pueden usar este comando." }, { quoted: msg });
      return;
    }
    ...
    if (estado === "on") {
      conn.setSubConfig(chatId, "chatgpt", 1);
    } else {
      conn.deleteSubConfig(chatId, "chatgpt");
    }

    // Se envían separados para evitar crasheos de estructura en Baileys
