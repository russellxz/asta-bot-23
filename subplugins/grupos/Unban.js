...
const handler = async (msg, { conn, args }) => {
  ...
  if (!rawTargets.size) {
    return conn.sendMessage(chatId, {
      text: "⚠️ Responde, menciona o escribe el número del usuario que quieres desbanear."
    }, { quoted: msg });
  }

  const welcomePath = path.join(conn.subDataDir, "setwelcome.json");
  ...
  for (const anyJid of rawTargets) {
    ...
    const forms = new Set([
      ...target.numbersToSave,
      DIGITS(anyJid),
      DIGITS(target.realJid),
      DIGITS(target.lidJid)
    ].filter(Boolean));
    ...
    const estaba = [...forms].some(n => bannedSet.has(n));

    if (estaba) {
      forms.forEach(n => bannedSet.delete(n));
      okLines.push(`@${showNumber}`);
    } else {
      noLines.push(`@${showNumber}`);
    }
  ...
  if (okLines.length) {
    texto += `✅ *Usuarios desbaneados correctamente:*\n${okLines.map((u, i) => `${i + 1}. ${u}`).join("\n")}\n\n`;
  }

  if (noLines.length) {
    texto += `⚠️ *Estos usuarios no estaban baneados:*\n${noLines.map((u, i) => `${i + 1}. ${u}`).join("\n")}`;
  }
