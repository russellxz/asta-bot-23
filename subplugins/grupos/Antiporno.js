// subplugins/grupos/Antiporno.js — activa el filtro de contenido +18
import fs from 'fs';
...
const handler = async (msg, { conn }) => {
  ...
  if (!["on", "off"].includes(estado)) {
    const activo = Number(conn.getSubConfig(chatId, "antiporno")) === 1;
...
⚠️ El bot necesita ser *administrador* para poder borrar y expulsar.`
  ...
  if (estado === "on") {
    conn.setSubConfig(chatId, "antiporno", 1);
  } else {
    conn.deleteSubConfig(chatId, "antiporno");
  }
